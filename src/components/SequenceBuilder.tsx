...
export function SequenceBuilder({ campaignId, onSave }: SequenceBuilderProps) {
  ...
  const addStep = (type: StepType) => {
    ...
    setSequenceSteps(prev => [
      ...prev,
      {
        ...
        message_template: type === 'email' && stepNumber === 1 ? DEFAULT_FIRST_EMAIL : '',
        stop_on_reply: true
      }
    ]);
    setMessage('');
  ...
  const saveSequence = async () => {
    ...
    const missingContent = sequenceSteps.find(
      step => !step.message_template.trim()
    );

    if (missingContent) {
      setMessage(
        `Step ${missingContent.step_number} needs its own ${missingContent.channel_type === 'voice' ? 'call script' : 'message copy'} before saving. Every sequence step is deterministic; nothing is generated by AI at send time.`
      );
      return;
    }
